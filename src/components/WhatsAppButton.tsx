...
export function WhatsAppButton() {
  ...
  // Mostra o balãozinho proativo após o usuário rolar 600px (engajado)
  useEffect(() => {
    if (dismissed) return;
    let triggered = false;
    const onScroll = () => {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50 flex items-end gap-3 md:bottom-7 md:right-7 [transform:translateZ(0)]">
      {/* Tooltip proativo — entrega hoje */}
      {showTooltip && !dismissed && (
        <div className="relative mb-1 max-w-[220px] animate-in fade-in slide-in-from-right-2 rounded-2xl border border-honey/30 bg-warm-brown-deep px-4 py-3 pr-8 shadow-[0_10px_30px_rgba(0,0,0,0.35)]">
          <button
            type="button"
            onClick={() => setDismissed(true)}
            aria-label="Fechar"
            className="absolute right-1.5 top-1.5 flex h-5 w-5 items-center justify-center rounded-full text-white/50 transition hover:bg-white/10 hover:text-white"
          >
            <X className="h-3 w-3" />
          </button>
          <p className="font-heading text-sm font-bold text-honey-gold">
          ...
          <span
            aria-hidden
            className="absolute -bottom-1.5 right-6 h-3 w-3 rotate-45 border-b border-r border-honey/30 bg-warm-brown-deep"
          />
        </div>
      )}
      ...
      >
        ...
        <span className="pointer-events-none hidden translate-x-2 rounded-full border border-honey/30 bg-warm-brown-deep/95 px-4 py-2 font-heading text-sm font-semibold text-honey-gold opacity-0 shadow-honey-lg transition-all duration-300 group-hover:translate-x-0 group-hover:opacity-100 md:inline-flex">
          Pedir meu mel 🍯
        </span>
